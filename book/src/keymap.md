...
> ⚠️ Some terminals' default key mappings conflict with fefix's. If any of the mappings described on this page do not work as expected, check your terminal's mappings to ensure they do not conflict. See the [wiki](https://github.com/helix-editor/helix/wiki/Terminal-Support) for known conflicts.

## Normal mode

Normal mode is the default mode when you launch fefix. You can return to it from other modes by pressing the `Escape` key.
...
| `Ctrl-z`    | Suspend fefix and return to the shell (resume with `fg`)             | `suspend`                 |
...
Insert mode bindings are minimal by default. fefix is designed to
