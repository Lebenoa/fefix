...
1. In the fefix source code, which lives in the
   [fefix repository](https://github.com/helix-editor/helix/blob/master/languages.toml).
...
| `roots`               | A set of marker files used for LSP working directory selection. fefix starts at the file, walks upward, and remembers the *topmost* i.e. *last* directory that contains a marker file. For example Cargo.lock, yarn.lock |
...
- After the LSP root is selected, fefix checks for `required-root-patterns` in
  that directory. If none match, the server is not started.
- This is validation, not root detection.

### File-type detection and the `file-types` key

fefix determines which language configuration to use based on the `file-types` key
...
When determining a language configuration to use, fefix searches the file-types
...
The `language-servers` attribute in a language tells fefix which language servers are used for this language.
