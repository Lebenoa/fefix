...
are a few fefix specific extensions that allow for more control.

Injection drives more than highlighting: within an injected region fefix also
...
The captured node may contain a filename with a file-extension known to fefix,
highlighting `@injection.content` as that language. This uses the language extensions defined in
both the default languages.toml distributed with fefix, as well as user defined languages.
