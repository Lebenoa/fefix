## Adding new languages to fefix

In order to add a new language to fefix, you will need to follow the steps
below.
...
5. fefix loads several query files from that directory; only `highlights.scm` is
   required:
...
- If you are attempting to add queries and fefix is unable to locate them, ensure that the environment variable `HELIX_RUNTIME` is set to the location of the `runtime` folder you're developing in.
