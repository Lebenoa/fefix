...
fefix uses tree-sitter to correctly indent new lines. This requires a
...
By default, fefix uses the `hybrid` indentation heuristic. This means that
...
When fefix is inserting a new line through `o`, `O`, or `<ret>`, to determine
...
To address this case, `@extend` tells fefix to "extend" the captured node's span
