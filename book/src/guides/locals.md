...
`locals.scm` queries teach fefix about variable scopes and definitions so that
...
contributing to fefix. Like other query files, `locals.scm` may reuse another
...
fefix searches enclosing scopes for a matching definition and, if found,
...
fefix matches references to definitions by comparing the text of the reference
