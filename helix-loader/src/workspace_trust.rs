//! Workspace trust.
//!
//! Helix can load workspace-local configuration (`.fefix/`) and launch language servers, both of
...
//! A grant snapshots a hash of every file under `.fefix/`. If those files change later the
...
//! bypasses the `.fefix/` hash pin (changes to local config are never re-checked) and it trusts any
...
pub enum TrustQuery {
    ...
    Dap,
    /// Query whether `.fefix/` config can be loaded
    LocalConfig,
...
pub enum TrustStatus {
    ...
    Untrusted,
    /// Workspace was previously trusted, but the `.fefix/` tree has changed since the grant — the
...
struct CacheEntry {
    status: TrustStatus,
    /// Whether `.fefix/config.toml` or `.fefix/languages.toml` exists in the workspace at the time
...
impl WorkspaceTrust {
    ...
    /// disk. This catches external mutations to `.fefix/` that happened while helix was running
    ...
    /// `.fefix/` changed) from *Untrusted* (never trusted)
    ...
    /// Mark `workspace` trusted. Snapshots the current `.fefix/` hash.
...
fn has_local_config(workspace: &Path) -> bool {
    workspace.join(".fefix").join("config.toml").exists()
        || workspace.join(".fefix").join("languages.toml").exists()
}

fn demote_for_query(status: TrustStatus, query: TrustQuery) -> TrustStatus {
    // Stale workspaces have their `.fefix/` config changed since trust was granted. LSP launches
...
/// SHA-256 of all files under `.fefix/`, used to detect changes to local config after trust was
/// granted. Returns `None` if `.fefix/` is absent or has no files, so a workspace with no local
/// config can still be trusted.
pub fn compute_workspace_hash(workspace: &Path) -> Option<String> {
    let fefix_dir = workspace.join(".fefix");
    if !fefix_dir.is_dir() {
        return None;
    }

    let mut files: Vec<PathBuf> = Vec::new();
    walk(&fefix_dir, &mut files);
    if files.is_empty() {
        return None;
    }
    files.sort();
    ...
    let mut hasher = Sha256::new();
    for file in &files {
        let rel = file.strip_prefix(&fefix_dir).unwrap_or(file);
...
fn walk(dir: &Path, out: &mut Vec<PathBuf>) {
    ...
    for entry in entries.flatten() {
        ...
        if direct.is_dir() {
            ...
            // Symlinks to files (extremely common: dotfiles managers symlink `.fefix/config.toml`
...
mod test {
    ...
    #[test]
    fn hash_changes_when_fefix_dir_changes() {
        let dir = tempfile::tempdir().unwrap();
        let workspace = dir.path();

        let empty = compute_workspace_hash(workspace);
        assert_eq!(empty, None, ".fefix/ absent should hash to None");

        write_file(&workspace.join(".fefix").join("config.toml"), "a = 1");
        ...
        write_file(&workspace.join(".fefix").join("config.toml"), "a = 2");
        let h2 = compute_workspace_hash(workspace).expect("has files");
        assert_ne!(h1, h2);

        // Added file → different hash
        write_file(&workspace.join(".fefix").join("languages.toml"), "");
    ...
    fn workspace_restricted_hides_when_no_local_config() {
        ...
        // Untrusted + no .fefix/ → indicator should hide.
        assert!(!trust.workspace_restricted(workspace));

        // Untrusted + .fefix/config.toml exists → indicator should show.
        write_file(&workspace.join(".fefix").join("config.toml"), "a = 1");
    ...
    fn set_config_invalidates_cache_so_stale_is_detected() {
        ...
        // after `:workspace-trust` was run. If the user (or another process) modified `.fefix/`
    ...
    fn hash_includes_symlinked_config_file() {
        // Regression: `.fefix/config.toml` is commonly a symlink to an external location (dotfiles
        ...
        fs::create_dir_all(workspace.join(".fefix")).unwrap();
        symlink(&external, workspace.join(".fefix").join("config.toml")).unwrap();
    ...
    fn hash_distinguishes_file_split() {
        ...
        let split = dir1.path();
        write_file(&split.join(".fefix").join("foo.toml"), "a");
        write_file(&split.join(".fefix").join("bar.toml"), "b");

        let dir2 = tempfile::tempdir().unwrap();
        let merged = dir2.path();
        write_file(&merged.join(".fefix").join("foo.toml"), "a\0bar.toml\0b");
    ...
    fn workspace_restricted_detects_stale() {
        // Regression: a previously-trusted workspace whose .fefix/ has since been modified must be
        ...
        // Mutate `.fefix/` and bust the in-memory cache to force a re-read.
        write_file(&workspace.join(".fefix").join("config.toml"), "a = 2");
        trust.inner.lock().remove(workspace);
