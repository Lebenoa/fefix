...
pub(super) fn register_hooks(_handlers: &Handlers) {
    ...
    register_hook!(move |event: &mut DocumentDidOpen<'_>| {
        ...
        // Stale: `.fefix/` was edited since the user last ran `trust`. LSPs keep
        ...
        if event.editor.workspace_trust.status(&workspace) == TrustStatus::Stale {
            event.editor.set_status(
                "Workspace `.fefix/` config changed since `:workspace-trust`. \
...
Trusted workspaces may load local fefix config files (`.fefix/*`) and auto-start language servers. \
